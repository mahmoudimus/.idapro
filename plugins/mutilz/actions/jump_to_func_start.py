# generated by mutilz cli at 2025-03-29 13:35:22
import dataclasses
import functools
import logging
import time
...
logger = logging.getLogger("mutilz.actions.jump_to_func_start")


def execute_action(current_ea: int):
    # get the function start ea
    func = ida_funcs.get_func(current_ea)
    if func is None or getattr(func, "start_ea", idaapi.BADADDR) == idaapi.BADADDR:
        print("No function found at current address.")
        return
    # jump to the function start
    idc.jumpto(func.start_ea)


@dataclasses.dataclass
class JumptofuncstartActionHandler(ida_helpers.BaseActionHandler):
    """Jump To Func Start"""

    action_name: str = "mutilz:jump_to_func_start"
    action_label: str = "Jump To Func Start"
    icon: int = 86
...
class JumptofuncstartAction(actions.action_t, metaclass=ida_helpers.HookedActionMeta):
    uihook_class = functools.partial(
        ida_helpers.PopUpHook,
        JumptofuncstartActionHandler,
