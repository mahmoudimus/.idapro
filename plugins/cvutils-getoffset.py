...
class cvutils_getoffset(idaapi.plugin_t):
    ...
    comment = "Get the (VA/File) Adress offset at the cursor location."
    help = "At a certain location right-click 'Get Offset' or 'Get File Offset'"
    ...
    ACTION_GET_FILEOFFSET = "prefix:get_fileoffset"
    ...
    def _init_action_get_offset(self):
        ...
        if idaapi.unregister_action(self.ACTION_GET_OFFSET):
            idaapi.msg(f"Warning: action {self.ACTION_GET_OFFSET} was already registered, unregistering it first\n")

        if idaapi.unregister_action(self.ACTION_GET_FILEOFFSET):
            idaapi.msg(
                f"Warning: action {self.ACTION_GET_FILEOFFSET} was already registered, unregistering it first\n"
            )
        ...
        if (sys.version_info > (3, 0)):
            ...
            action_desc2 = idaapi.action_desc_t(
                self.ACTION_GET_FILEOFFSET,  # The action name.
                "Get File Offset",  # The action text.
                IDACtxEntry(getcopy_fileoffset),  # The action handler.
    ...
    def hxe_callback(self, event, *args):
        ...
        if event == idaapi.hxe_populating_popup:
            ...
            idaapi.attach_action_to_popup(
                form,
                popup,
                cvutils_getoffset.ACTION_GET_FILEOFFSET,
                "Get File Offset",
                idaapi.SETMENU_APP,
            )
...
def getcopy_fileoffset():
    """
    Gets the offset of the current cursor's address
    """
    vCurrentPos = get_screen_linear_address()
    if vCurrentPos != idaapi.BADADDR:
        fOffset = idaapi.get_fileregion_offset(vCurrentPos)
        print(
            "Address [0x%x] =-> File Input Offset [%x] Copied to Clipboard!"
            % (vCurrentPos, fOffset)
        )
        setClipboardText("%x" % fOffset)
    return
