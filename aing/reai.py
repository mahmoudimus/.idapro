...
def exception_code_check(ea, caller_ea_arg=0):
    ...
    if caller_ea_arg != 0 and (
        ea in processed_func
        or ea in processing_func
        or func == None
        or "sub_" not in func_name
    ):
        return True
...
def get_call_topology(ea, caller_ea, parent_ea, parent_chain, count):
    ...
    for caller_ea, call_ea in sub_func.items():
        ...
        if (
            current_node.child.get(call_ea) == None
            and call_ea not in current_node.parent_chain
            and call_ea != ea
            and func != None
            and "sub_" in func_name
        ):
    ...
    for call_ea, caller_ea in current_node.child.items():
        if function_info.get(call_ea) == None:
            ...
            function_info[call_ea].parent_chain = (
...
def AI_ananalyze(func_start):
    ...
    while len(function_info[func_start].child) != 0:
        ...
        for ea, func_node in function_info.items():
            if len(func_node.child) == 0:
                ...
                sub_thread = threading.Thread(
                    target=AI_work, args=(func_node.ea, str(c_func), func_name)
                )
        ...
        print(
            f'[REAI] Finish round {count}, parse list: {", ".join([hex(addr) for addr in round_list])}'
        )
        count += 1
...
class exception_code_check_action(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        if exception_code_check(ea) != True:
            ...
            print(
                f"[REAI] Exception code is not exist, all function num: {len(processed_func)}"
            )
        exception_code_collection = []
        processed_func = []
    ...
    def finish_populating_widget_popup(self, form, popup):
        ...
        if idaapi.get_widget_type(form) == idaapi.BWN_PSEUDOCODE:
            ...
            idaapi.attach_action_to_popup(
                form, popup, "REAI:exception_code_check", "REAI/"
            )
            idaapi.attach_action_to_popup(
                form, popup, "REAI:call_topology_print", "REAI/"
            )
