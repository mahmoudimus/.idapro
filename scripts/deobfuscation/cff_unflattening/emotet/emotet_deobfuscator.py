...
class CEmotetCFF(optblock_t):
        ...
        for i in range(1, blockCnt - 1):
            ...
            while mins != currentBlock.tail:
                    ...
                    if get_mreg_name(mins.d.r, mins.d.size) == dispatchReg:
                        ...
                        if status in statusList:
                            ...
                            if currentBlock.tail.opcode == m_goto:
                                ...
                                minsGoto.d = mop_t()
                                minsGoto.iprops = 0
                                currentBlock.insert_into_block(
                                    minsGoto, currentBlock.tail
                                )
                ...
                if (
                    ...
                    and mins.r.t == mop_n
                    and mins.d.t == mop_r
                    and get_mreg_name(mins.d.r, mins.d.size) == dispatchReg
                ):
                    subMins1 = mins.l.d
                    if (
                        subMins1.opcode == m_and
                        and subMins1.l.t == mop_d
                        and subMins1.r.t == mop_n
                    ):
                        ...
                        if subMins2.opcode == m_neg and subMins2.l.t == mop_d:
                            ...
                            if subMins3.opcode == m_xdu and subMins3.l.t == mop_d:
                                ...
                                if (
                                    subMins4.opcode == m_setnz
                                    and subMins4.l.t == mop_r
                                    and subMins4.r.t == mop_n
                                    and subMins4.r.value(False) == 0
                                ):
                                    status1 = mins.r.value(False)
                                    status2 = (
                                        mins.r.value(False) + subMins1.r.value(False)
                                    ) & 0xFFFFFFFF
                                    if status1 in statusList and status2 in statusList:
                                        ...
                                        for oldDstBlockSerial in currentBlock.succset:
                                            oldDstBlock = mba.get_mblock(
                                                oldDstBlockSerial
                                            )
                                            oldDstBlock.predset._del(
                                                currentBlock.serial
                                            )
                                        currentBlock.succset.clear()
                                        ...
                                        if (
                                            currentBlock.serial
                                            not in mba.get_mblock(
                                                newDstBlockSerial
                                            ).predset
                                        ):
                                            mba.get_mblock(
                                                newDstBlockSerial
                                            ).predset.push_back(currentBlock.serial)
                                        ...
                                        insertedJnzIns.r.make_number(
                                            subMins4.r.value(False), 4
                                        )
                                        insertedJnzIns.d = mop_t()
                                        insertedJnzIns.d.make_blkref(newDstBlockSerial)
                                        currentBlock.insert_into_block(
                                            insertedJnzIns, currentBlock.tail
                                        )
                                        ...
                                        currentBlock.succset.push_back(
                                            newInsertedBlock.serial
                                        )
                                        newInsertedBlock.predset.push_back(
                                            currentBlock.serial
                                        )
                                        newGotoBlockSerial = statusList[status1]
                                        mba.get_mblock(
                                            newGotoBlockSerial
                                        ).predset.push_back(newInsertedBlock.serial)
                                        insertedGotoIns = minsn_t(currentBlock.tail.ea)
                                        insertedGotoIns.opcode = m_goto
                                        insertedGotoIns.l._make_blkref(
                                            newGotoBlockSerial
                                        )
                                        insertedGotoIns.r = mop_t()
                                        insertedGotoIns.d = mop_t()
                                        newInsertedBlock.insert_into_block(
                                            insertedGotoIns, None
                                        )

                                        newInsertedBlock.succset.push_back(
                                            newGotoBlockSerial
                                        )
    ...
    def func(self, blk):
        ...
        for k, v in dispatchDict.items():
            if (
                v >= 3
            ):  # If reg was compare with const at least 3 times, consider it's dispatch reg
...
class CDispatchCleaner(minsn_visitor_t):
    ...
    def visit_minsn(self):
        ...
        if ins.opcode in [m_jz, m_jnz]:
            if ins.r.t == mop_n and ins.l.t == mop_r:
                if (
                    self.dispatchReg == get_mreg_name(ins.l.r, ins.l.size)
                    and ins.r.value(False) in self.fixStatusList
                ):
                    # Now we clean block
                    jmpDst = ins.d.b
                    if ins.opcode == m_jz:
