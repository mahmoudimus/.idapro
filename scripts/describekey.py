...
class ActionInfo(object):
    ...
    def __init__(
        self, label, action, shortcut, tooltip, state, visibility, checkable, checked
    ):
        self.label = label
        self.action = action
    ...
    def for_action(cls, action):
        ...
        return ActionInfo(
            label, action, shortcut, tooltip, state, visibility, checkable, checked
        )
...
class DescribeKey(object):
    ...
    def _handle_keyevent(self, event: QKeyEvent, action_map, fn):
        ...
        if DEBUG:
            print(
                "evt: {}  key: {:7X}  native key: {:2X}  native scancode: {:2X}  "
                "text: {:1}  shortcut: {} modifiers: {} key2str: {}".format(
                    type(event),
                    event.key(),
    ...
    def _build_table(self) -> QTableWidget:
        ...
        table.setHorizontalHeaderItem(
            COL_SHORTCUT, QtWidgets.QTableWidgetItem("Shortcut")
        )
        table.setHorizontalHeaderItem(
            COL_TOOLTIP, QtWidgets.QTableWidgetItem("Description")
        )
