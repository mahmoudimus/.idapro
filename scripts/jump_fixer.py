...
def explore_function(start_ea, bs):
    loc_db = LocationDB()
    # bs = bin_stream_ida()
    ...
    solved_addr = z3.BitVec("solved_addr", 64)
    for x in readonly_regions:
        for y in range(x[0], x[1] + 8, 8):
            var_bytes = bs.getbytes(y, 8)[::-1]
            value = int(encode_hex(var_bytes), 16)
            s.add(z3_mem.get(z3.BitVecVal(y, 64), 64) == value)
    ...
    while len(branches_to_explore) > 0:
        ...
        if isinstance(simp_addr, ExprInt):
            ...
            while s.check() == z3.sat:
                ...
                solution = sol.as_long() # type: ignore
...
def patch_unconditional_blocks(todo, bs):
    loc_db = LocationDB()
    #bs = bin_stream_ida()
    machine: Machine = Machine("x86_64")
    mdis = machine.dis_engine(bs, loc_db=loc_db) # type: ignore
...
def patch_conditional_blocks(todo, bs):
    loc_db = LocationDB()
    #bs = bin_stream_ida()
    machine = Machine("x86_64")
    mdis = machine.dis_engine(bs, loc_db=loc_db) # type: ignore
    lifter = machine.lifter_model_call(loc_db) # type: ignore
...
if __name__ == "__main__":
    ...
    unconditional_blocks, conditional_blocks = explore_function(func.start_ea, bs)
    ...
    patch_unconditional_blocks(unconditional_blocks, bs)
    patch_conditional_blocks(conditional_blocks, bs)
