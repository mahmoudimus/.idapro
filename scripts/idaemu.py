...
class Emu(object):
    def __init__(self, arch, mode, compiler=COMPILE_MSVC, stack=0xf000000,
                 ssize=3):
    ...
    def _getOriginData(self, address, size):
        ...
        for offset in range(0, size, 64):
            tmp = ida_bytes.get_bytes(address + offset, 64)
            if tmp == None:
                res.extend([pack("<Q", ida_bytes.get_qword(address + offset + i)) for i in range(0, 64, 8)])
            else:
                res.append(tmp)
        res = b"".join(res)
        return res[:size]
    ...
    def eFunc(self, address=None, retAddr=None, args=[]):
        ...
        if retAddr == None:
            refs = [ref.frm for ref in idautils.XrefsTo(func.startEA, 0)]
            if len(refs) != 0:
                retAddr = refs[0] + ida_bytes.get_item_size(refs[0])
            else:
    ...
    def eBlock(self, codeStart=None, codeEnd=None):
        if codeStart == None: codeStart = idc.read_selection_start()
        if codeEnd == None: codeEnd = idc.read_selection_end()
    ...
    def eUntilAddress(self, startAddr, stopAddr, args=[], TimeOut=0, Count=0):
        ...
        # ida_bytes.get_cmt out 
        #self._showRegs(self.curUC)
        
    
    def get_reg_value(self, reg_def):
        ret_reg_value = self.curUC.reg_read(reg_def)
        return ret_reg_value
        
    def get_decrypt_string(self):
        return self.decrypt_string

    def get_curr_uc(self):
        return self.curUC
        
    def getEmuString(self, addr):
        out = ""
        while str(self.curUC.mem_read(addr, 1)) != "\x00":
            out += str(self.curUC.mem_read(addr, 1))
            addr += 1
        return out
    def getEmuWideString(self, addr):
        out = ""
        while str(self.curUC.mem_read(addr, 2)) != "\x00\x00":
            out += str(self.curUC.mem_read(addr, 1))
            addr += 2
        return out
