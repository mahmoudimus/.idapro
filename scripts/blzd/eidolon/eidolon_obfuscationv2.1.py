...
def resolve_overlaps(ranges: list[Range], progress_callback=None) -> IntervalSet:
    """
    Fast, linear-time overlap resolution: keep only the first chain
    whose start is ≥ the furthest end so far.

    Args:
        ranges: List of Range objects to resolve
        progress_callback: Optional callback function(current_index, total) for progress updates
    """
    total_ranges = len(ranges)
    logging.info(f"Resolving overlaps among {total_ranges} ranges")

    # Update progress before sorting (sorting can be slow for large datasets)
    if progress_callback:
        progress_callback(0, total_ranges)
        QApplication.processEvents()

    intervals = IntervalSet()
    # Sort ranges by start address and remove duplicates
    # This can be slow for very large lists, but we can't easily show progress during sort
    ranges = sorted(set(ranges), key=lambda x: x.start)
    total_ranges = len(ranges)  # Update count after deduplication

    # Update progress after sorting
    if progress_callback:
        progress_callback(0, total_ranges)  # Signal that sorting is done
        QApplication.processEvents()

    for idx, r in enumerate(ranges):
        # Update progress more frequently for better responsiveness
        if progress_callback and (idx % 1000 == 0 or idx == total_ranges - 1):
            progress_callback(idx, total_ranges)
...
class CapstoneAnalysisRunnable(QRunnable):
    def __init__(self, data_chunk, analysis_engine):
        ...
        # Store read-only data that can be safely shared
        self.junk_detector = analysis_engine.junk_detector  # Read-only, safe to share

    def run(self):
        self.signals.auto_started.emit()
        try:
            # Create thread-local Capstone instance (Capstone is NOT thread-safe)
            thread_local_cs = capstone.Cs(capstone.CS_ARCH_X86, capstone.CS_MODE_64)
            thread_local_cs.detail = True

            # Create a temporary engine-like object with thread-local Capstone
            # We'll call the analysis methods directly with our thread-local instance
            chunk_matches = []
            items_processed_in_this_chunk = 0
            progress_batch_size = (
                100  # Emit progress every 100 items instead of every item
            )

            for candidate_ea, region_bytes, base_address in self.data_chunk:
                # Use thread-local Capstone instance instead of shared one
                matches = self._analyze_candidate_with_local_cs(
                    region_bytes, base_address, candidate_ea, thread_local_cs
                )
                chunk_matches.extend(matches)
                items_processed_in_this_chunk += 1

                # Batch progress updates to reduce signal emission overhead
                if items_processed_in_this_chunk % progress_batch_size == 0:
                    self.signals.progress.emit(progress_batch_size)

            # Emit remaining progress
            remaining = items_processed_in_this_chunk % progress_batch_size
            if remaining > 0:
    ...
    def _analyze_candidate_with_local_cs(
        self, region_bytes, base_address, candidate_ea, cs_instance
    ):
        """Analyze candidate using thread-local Capstone instance."""
        matches = []
        if not region_bytes:
            return matches
        ...
        try:
            ...
            matches = self._find_patterns_in_bytes(
                analysis_bytes, base_address + analysis_start, cs_instance
            )
        except Exception as e:
            logging.warning("Error analyzing candidate at 0x%x: %s", candidate_ea, e)
    ...
    def _find_patterns_in_bytes(
        self, data: bytes, base_address: int, cs_instance
    ) -> List[PatternMatch]:
        """Find patterns using provided Capstone instance (thread-local)."""
        patterns = []
        if not USE_CAPSTONE or cs_instance is None:
            return patterns

        try:
            instructions = list(cs_instance.disasm(data, base_address))
...
class ControlPanel(QGroupBox):
    ...
    def __init__(self, parent=None):
        ...
        self.dry_run_checkbox.setChecked(True)  # Default to dry run enabled
        self.dry_run_checkbox.setToolTip(
            "When checked, patterns are detected but not patched"
        )
        ...
        self.patch_btn = QPushButton("Patch Results")
        ...
        self.layout.addWidget(QLabel("|"))
        self.layout.addWidget(self.patch_btn)
...
class PatternDetectionWidget(QWidget):
    ...
    def _connect_signals(self):
        ...
        self.control_panel.patch_btn.clicked.connect(self.patch_current_results)
    ...
    def analysis_finished(self, unique_matches: List[PatternMatch]):
        ...
        # Wait for tasks with progress updates to keep UI responsive
        total_tasks = len(analysis_tasks_submitted)
        self.progress_display.progress_label.setText(
            f"Analyzing jump chains: processing {total_tasks} tasks..."
        )
        QApplication.processEvents()

        # Wait for completion with periodic UI updates to prevent freezing
        # Timeout allows proceeding with partial results if analysis takes too long
        ...
        while thread_pool.activeThreadCount() > 0:
            ...
            if elapsed > max_wait_time:
                active = thread_pool.activeThreadCount()
                completed = total_tasks - active
                logging.warning(
                    "Task completion timeout reached after %d minutes. "
                    "Proceeding with %d/%d completed tasks. "
                    "You can re-run the script to process remaining tasks.",
                    int(elapsed / 60),
                    completed,
                    total_tasks,
                )
                self.progress_display.progress_label.setText(
                    f"Timeout reached: {completed}/{total_tasks} tasks completed. Proceeding with available results..."
                )
                QApplication.processEvents()
                break
            ...
            if elapsed - last_update >= 0.5:
                ...
                remaining_min = int((max_wait_time - elapsed) / 60)
                self.progress_display.progress_label.setText(
                    f"Analyzing jump chains: {completed}/{total_tasks} completed ({active} active) - {elapsed_min}m {elapsed_sec}s elapsed (~{remaining_min}m remaining)..."
                )
                last_update = elapsed
        ...
        # Final wait to ensure all tasks are truly done (with timeout)
        thread_pool.waitForDone(5000)  # 5 second timeout for final cleanup

        logging.debug("All tasks completed. Collecting results...")
        self.progress_display.progress_label.setText("Collecting analysis results...")
        QApplication.processEvents()

        # Optimized collection: collect lists first, then flatten in one pass
        # This avoids many small extend() operations which cause list reallocations
        error_count = 0
        last_update_time = time.time()
        update_interval = 0.2  # Update UI every 200ms
        range_lists = []  # Collect all result_ranges lists first

        # First pass: collect all range lists (just references, very fast)
        for i, task in enumerate(analysis_tasks_submitted):
            # Time-based updates to avoid overhead
            current_time = time.time()
            if current_time - last_update_time >= update_interval:
                self.progress_display.progress_label.setText(
                    f"Collecting results: {i}/{total_tasks} tasks processed..."
                )
                QApplication.processEvents()
            ...
            if task.error:
                error_count += 1
                # Only log first few errors to reduce overhead
                if error_count <= 5:
                    logging.warning(
                        "Task %d for pm at 0x%X encountered an error: %s.",
                        i + 1,
                        task.pm_ida_address,
                        task.error,
                    )
            else:
                # Collect the list reference (very fast, no copying)
                if task.result_ranges:
                    range_lists.append(task.result_ranges)

        # Second pass: flatten all lists at once using itertools (much faster)
        if range_lists:
            self.progress_display.progress_label.setText(
                f"Flattening {len(range_lists)} result lists..."
            )
            QApplication.processEvents()
            # Use itertools.chain to flatten all lists efficiently
            # This is much faster than many small extend() operations
            all_resolved_ranges = list(chain.from_iterable(range_lists))
        else:
            # No ranges collected, keep as empty list
            all_resolved_ranges = []

        if error_count > 5:
            logging.warning(
                "Total of %d tasks encountered errors (only first 5 logged above).",
                error_count,
            )
        ...
        self.progress_display.progress_label.setText(
            f"Resolving overlaps among {len(all_resolved_ranges)} ranges..."
        )
        QApplication.processEvents()

        def progress_callback(current, total):
            # Update more frequently for better responsiveness
            # The function itself checks every 1000 items, so we'll see updates regularly
            self.progress_display.progress_label.setText(
                f"Resolving overlaps: {current}/{total} ranges processed..."
            )
            QApplication.processEvents()

        final_interval_set = resolve_overlaps(all_resolved_ranges, progress_callback)
        logging.info(
            "Overlap resolution complete. Final intervals in set: %d",
            len(final_interval_set),
        )
        # Use checkbox state to determine if we should actually patch
        dry_run = self.control_panel.dry_run_checkbox.isChecked()
        processed_patterns_for_display: List[PatternMatch] = []

        if final_interval_set:
            if dry_run:
                # In dry run mode, skip patch creation entirely - just create display patterns
                logging.info(
                    "Dry run mode: Skipping patch creation. Found {} patterns for display only.".format(
                        len(final_interval_set)
                    )
                )
                ...
                # Normal mode: create patches and apply them
                ...
                for resolved_range in final_interval_set:
                    ...
                    if new_pm:
                        processed_patterns_for_display.append(new_pm)
                logging.info(
                    "Analysis completed. Found {} patch operations.".format(patch_count)
                )
    ...
    def patch_current_results(self):
        """Patch the currently displayed results."""
        if not self.all_patterns:
            QMessageBox.information(self, "Patch Results", "No patterns to patch.")
            return

        reply = QMessageBox.question(
            self,
            "Patch Results",
            f"Apply patches to {len(self.all_patterns)} patterns?\n\n"
            "This will replace obfuscated code with NOPs.\n"
        ...
        try:
            ...
            for pattern in self.all_patterns:
                start_addr = pattern.ida_address
                length = pattern.total_length
                if length > 0:
                    patch_manager.add_patch(start_addr, b"\x90" * length)
                    patch_count += 1
                    total_bytes += length

            if patch_count == 0:
                QMessageBox.warning(
                    self, "Patch Results", "No valid patterns to patch."
                )
                return

            # Apply all patches
            success = patch_manager.apply_all()

            if success:
                QMessageBox.information(
                    self,
                    "Patch Complete",
                    f"Successfully applied {patch_count} patches ({total_bytes} bytes).",
                )
                logging.info(
                    "Applied %d patches (%d bytes) from current results.",
                    patch_count,
                    total_bytes,
                )
            else:
                QMessageBox.warning(
                    self,
                    "Patch Warning",
                    f"Applied patches with some errors. Check logs for details.",
                )

        except Exception as e:
            QMessageBox.critical(
                self, "Patch Error", f"Failed to apply patches:\n{str(e)}"
            )
            logging.error("Error patching results: %s", e, exc_info=True)
    ...
    def update_buttons_state(self):
        ...
        self.control_panel.patch_btn.setEnabled(has_patterns)
