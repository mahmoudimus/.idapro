...
class JumpTargetAnalyzer:
    def __init__(
        ...
        self.jump_targets = collections.Counter()
        self.jump_details = []
    ...
    def follow_jump_chain(self, mem, current_ea, match_end, visited=None):
        if visited is None:
            visited = set()
        if current_ea in visited or not (
            self.start_ea <= current_ea < self.start_ea + len(mem.mem_results)
        ):
            return None
        visited.add(current_ea)

        offset = current_ea - self.start_ea
        code = mem.mem_results[offset:]

        # disassemble only one insn
        for insn in self.cs.disasm(code, current_ea, count=1):
            # skip NOPs
            if insn.id == 0x90:  # x86 nop
                return self.follow_jump_chain(mem, current_ea + 1, match_end, visited)

            # test for conditional/unconditional jump
            if insn.group(CS_GRP_JUMP) and len(insn.bytes) in (2, 5):
                ...
                if self.match_start <= target < match_end + 6:
                    ...
                    return target
                return (
                    target if (self.start_ea <= target < match_end + 6) else current_ea
                )

            # not a jump we care about
            break
...
def stage1_find_patterns(mem_bytes: bytes, base_ea: int) -> List[MatchChain]:
    """Scan for all stage1 patterns (multi‑part & single) in parallel over patterns."""
    # flatten (pattern_list, desc, segtype)
    jobs = []
    for pattern_group, desc, segtype in [
        (MULTI_PART_PATTERNS, "Multi", SegmentType.STAGE1_MULTIPLE),
        (SINGLE_PART_PATTERNS, "Single", SegmentType.STAGE1_SINGLE),
    ]:
        for pat in pattern_group:
            jobs.append((pat.pattern, desc, segtype))

    def scan_one(args):
        pat_bytes, desc, segtype = args
        prog = re.compile(pat_bytes, re.DOTALL)
        chains = []
        for m in prog.finditer(mem_bytes):
            # … build a MatchChain for each match (same as in find_stage1) …
            chains.append( … )
        return chains

    with ProcessPoolExecutor() as exe:
        results = exe.map(scan_one, jobs)
    # flatten
    all_chains = [c for sub in results for c in sub]
    all_chains.sort()
    return all_chains

def stage2_append_junk(mem_bytes: bytes, chains: List[MatchChain]) -> List[MatchChain]:
    def junk_for_chain(chain: MatchChain):
        # copy and call your per‑chain junk logic here
        find_junk_instructions_after_stage1_for_chain(mem_bytes, chain)
        return chain

    with ProcessPoolExecutor() as exe:
        return list(exe.map(junk_for_chain, chains))


def stage3_filter_chains(chains: List[MatchChain]) -> List[MatchChain]:
    # This is fast enough to do in‑process, but you *can* parallelize by per‑chain filter
    return [c for c in chains if c.junk_length and 12 <= c.overall_length() <= 129]


def stage4_validate_and_detect_big(mem_bytes: bytes, chains: List[MatchChain], base_ea: int) -> List[MatchChain]:
    def validate_chain(chain: MatchChain):
        # replicate your filter_antidisasm_patterns big‑instr logic *per chain*
        # using JumpTargetAnalyzer on chain.overall_matched_bytes()
        # append segment if found, or drop chain
        return maybe_chain_or_None

    with ProcessPoolExecutor() as exe:
        validated = exe.map(validate_chain, chains)
    return [c for c in validated if c]
        ...
        try:
            ...
            with SharedMemoryChunk(shm_name) as shm_chunk:
                ...
                with shm_chunk.buf[offset : offset + chunk_size] as chunk_view:
                    ...
                    base_ea = 0  # or pass this in if you need the actual load address

                    # 1️⃣ Stage 1:
                    chains1 = stage1_find_patterns(chunk_view, base_ea)

                    # 2️⃣ Stage 2:
                    chains2 = stage2_append_junk(mem_buf, chains1)

                    # 3️⃣ Stage 3:
                    chains3 = stage3_filter_chains(chains2)

                    # 4️⃣ Stage 4:
                    final_chains = stage4_validate_and_detect_big(mem_buf, chains3, base_ea)

                    # Build patch list:
                    results = []
                    for chain in final_chains:
                        results.append({
                            "offset": chain.overall_start(),
